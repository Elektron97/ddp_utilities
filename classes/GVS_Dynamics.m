    ...
    methods
        ...
        function [fx, fu] = discretized_gradients(obj, t, x, u, h, options)
            arguments
                ...
                h
                options.method = "ode1"
                options.grad_method = "analytical" % "analytical" or "central_difference"
                options.eps = 1e-6                 % Step size for finite difference
            end
            
            %% Numerical Estimation (Central Difference)
            if options.grad_method == "central_difference"
                fx = zeros(obj.nx, obj.nx);
                fu = zeros(obj.nx, obj.nact);
                
                % Finite differences with respect to state (x)
                for i = 1:obj.nx
                    dx = zeros(obj.nx, 1);
                    dx(i) = options.eps;
                    % Evaluate discretized dynamics at (x + dx) and (x - dx)
                    x_plus  = obj.discretize_dynamics(t, x + dx, u, h, "method", options.method);
                    x_minus = obj.discretize_dynamics(t, x - dx, u, h, "method", options.method);
                    % Central difference
                    fx(:, i) = (x_plus - x_minus) / (2 * options.eps);
                end
                
                % Finite differences with respect to input (u)
                for i = 1:obj.nact
                    du = zeros(obj.nact, 1);
                    du(i) = options.eps;
                    % Evaluate discretized dynamics at (u + du) and (u - du)
                    x_plus  = obj.discretize_dynamics(t, x, u + du, h, "method", options.method);
                    x_minus = obj.discretize_dynamics(t, x, u - du, h, "method", options.method);
                    % Central difference
                    fu(:, i) = (x_plus - x_minus) / (2 * options.eps);
                end
                
                return; % Early exit once numerical gradients are computed
            end
            
            %% Gradients of the Discretized System (Analytical)
            switch(options.method)
                case "ode1"
                    % Compute Analytical Derivatives (Continuous System)
...
%% Helper Functions (Outside the Classdef)
